texts = []
#texts = ["\ue00cPlay 2 Games to Get Registered\ue00c", "Only Top 5 Players of this server can only use custom Tag\n Create a profile with '/tag<space>' as prefix\n Ex : '/tag MyCustomTag'", "\ue046DO NOT ASK FOR ADMINSHIP\ue046", "Visit eoni.ml website or join Discord @ \nhttp://discord.io/EoniOfficial \ngroup to know more about us !"]

# Released under the MIT License. See LICENSE for details.
#
"""Map related functionality."""
import ba,_ba,random,mysettings
from mysettings import settings, uni, screenTextColor, multicolor
from bastd.gameutils import SharedObjects
from typing import Any, List, Dict
from bastd import maps as bastd_maps
from bastd.maps import *

our_texts = texts if texts else mysettings.texts
...
def showtext(tex, c):
    for k,v in uni.items():
        if k in tex:
            tex = tex.replace(k,v)
    t = ba.newnode('text',attrs={'text':tex.encode('utf-8').decode('unicode-escape'),'color': screenTextColor if screenTextColor else (1,1,1),'maxwidth':0,'position':(0,140),'shadow':0.4,'flatness':0.8,'h_align':'center','v_attach':'bottom'})
    ba.animate(t,'opacity',{0:0,c-1001:0,c-1000:0.5,c:1.0}, timetype=ba.TimeType.SIM, timeformat=ba.TimeFormat.MILLISECONDS)
    ba.animate(t,'scale',{0:0,c-1001:0,c-1000:0.5,c:0.8}, timetype=ba.TimeType.SIM, timeformat=ba.TimeFormat.MILLISECONDS)
    if not screenTextColor:
        ba.animate_array(t,'color',3,multicolor,True,timetype=ba.TimeType.SIM,timeformat=ba.TimeFormat.MILLISECONDS)
    d_timer = ba.timer(((c+13000) / 1000),t.delete)
mytext = showtext
def startShowing():
    b = 1000
    for text in our_texts:
        mytext(text, b)
        b += 15000
if settings['screenTexts']:
    starter = ba.Timer(500, ba.Call(startShowing), timeformat=ba.TimeFormat.MILLISECONDS)
    runner = ba.Timer(len(our_texts)*15500, ba.Call(startShowing), timeformat=ba.TimeFormat.MILLISECONDS, repeat = True)
def checkSettingsFile():
    global starter
    global runner
    global mytext
    if not settings['screenTexts']:
        starter = None
        runner = None
    if (settings['screenTexts']) and (None in [starter, runner]):
        starter = ba.Timer(500, ba.Call(startShowing), timeformat=ba.TimeFormat.MILLISECONDS)
        runner = ba.Timer(len(our_texts)*15500, ba.Call(startShowing), timeformat=ba.TimeFormat.MILLISECONDS, repeat = True)
checkSettingsFileTimer = ba.Timer(10, ba.Call(checkSettingsFile), repeat = True)'''

#############################################################################################################################################

class myHockeyStadium(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        self.is_hockey = True
        exec(myscript)
bastd_maps.HockeyStadium = myHockeyStadium


class myFootballStadium(ba.Map):
...
bastd_maps.FootballStadium = myFootballStadium


class myBridgit(ba.Map):
...
class myBigG(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        gnode.vignette_inner = (0.9, 0.9, 0.93)
        exec(myscript)
bastd_maps.BigG = myBigG


class myRoundabout(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.Roundabout = myRoundabout


class myMonkeyFace(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.MonkeyFace = myMonkeyFace


class myZigZag(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.ZigZag = myZigZag


class myThePad(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.ThePad = myThePad


class myDoomShroom(ba.Map):
...
bastd_maps.DoomShroom = myDoomShroom


class myLakeFrigid(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        self.is_hockey = True
        exec(myscript)
bastd_maps.LakeFrigid = myLakeFrigid


class myTipTop(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.TipTop = myTipTop


class myCragCastle(ba.Map):
    """A lovely castle map."""
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.CragCastle = myCragCastle


class myTowerD(ba.Map):
...
bastd_maps.TowerD = myTowerD


class myHappyThoughts(ba.Map):
    """Flying map."""
    ...
    def __init__(self) -> None:
        ...
        exec(myscript)
bastd_maps.HappyThoughts = myHappyThoughts


class myStepRightUp(ba.Map):
    ...
    def __init__(self) -> None:
        ...
        gnode.vignette_inner = (0.95, 0.95, 0.93)
        exec(myscript)
bastd_maps.StepRightUp = myStepRightUp


class myCourtyard(ba.Map):
    ...
    def is_point_near_edge(self,
        ...
        zpos = (point.z - box_position[2]) / box_scale[2]
        return xpos < -0.5 or xpos > 0.5 or zpos < -0.5 or zpos > 0.5
bastd_maps.Courtyard = myCourtyard


class myRampage(ba.Map):
