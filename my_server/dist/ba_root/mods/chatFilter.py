white = ['pb-IF40V1hYXQ==']
#just an example on how to add..  remove my if u want...
...
chatCoolDown = {}
whiteListedPeeps = roles.owners + roles.admins + white
abusers={}
...
abuselist3='bitch xxx erotic shit nipple jhaat mc bhosdika baap aulad chood chhod bhadwe bhadve behnchod gaand baap'
        ...
        if (clientID != -1) and (i != {}) and (i['client_id'] == clientID):
            ...
            with open(chatLogFile, 'w') as f:
                for i in chatlogs:
                    f.write(i)
                f.close()
            lastm = {str(clog):[l_n, l_m, l_ds, uID]}
            ...
            for f in sp:
                if settings['enableChatFilter'] and (uID not in roles.chatWhitelist) and ('**' in message): # or (f.lower() in abuselist.split()) or (f.lower() in abuselist2.split()) or (f.lower() in abuselist3.split()) or (f.lower() in abuselist4.split(', ')):
                    warnCount = warn(uID)
                    if warnCount < 3:
                        _ba.chatmessage(f"Abuse detected. Warning {str(name)}.")
                        ...
                        #_ba.chatmessage("Abuse Filter Made By Ankit") #Disabled This cuz, i used Mr.Smoothy's too
