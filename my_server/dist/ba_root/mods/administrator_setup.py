settings = {'enableChatFilter': True, 'enableTop5effects': True, 'enableTop5commands': False, 'enableCoinSystem': True, 'pUpLight': True, 'pUpTag': True, 'pUpShield': True, 'screenTexts': True, 'statsEndDate': 25, 'whiteListMode': False, 'chatWhitelistMode': False, 'showHP': True, 'hitTexts': True, 'hideCmds': False, 'stats':True, 'askQuestions':True}
...
party_queue_id = None #Replace with queue_id of this server as a string, this is used for queueChecker.py

#anounce when a player was tried to kick but was in whitelist and failed to  kick
...
#
htmlFile = f"{python_path}{step}mydata{step}stats.html" #'/var/www/mydomainname.ml/stats.html'
#Log file of players contains their device ids
playerLogFile = f"{python_path}{step}logs{step}PlayerLogs.json" #'/home/ubuntu/folderName/logs/PlayerLogs.json'
...
statsFile = f"{python_path}{step}mydata{step}stats.json" #'/home/ubuntu/folderName/stats.json'
#bankFile contains their server currency data
bankFile = f"{python_path}{step}mydata{step}bank.json" #'/home/ubuntu/folderName/bank.json'
#this contains of the various people in roles.py (owners,admins,banned players,etc)
membersFile = f"{python_path}{step}mydata{step}members.json" #'/home/ubuntu/folderName/members.json'
#this contains only the successful commands executed by people in the server
cmdLogFile = f"{python_path}{step}logs{step}cmd_logs.txt" #'/home/ubuntu/folderName/logs/cmd_logs.json'
#this contains all chats sent in server including cmds
chatLogFile = f"{python_path}{step}logs{step}chatLogs.txt" #'/home/ubuntu/folderName/logs/chatLogs.txt'
#this will have only last msg sent in server (Useable for live status data)
lastMsgFile = f"{python_path}{step}logs{step}last_message.json" #'/home/ubuntu/folderName/logs/last_message.json'
...
#Add extra white list here (account_id)
#Should add pb_id (Note: Owners and admins are defaultly added through scripts)
RJCDwhiteList = []
...
questionsList = {'Who is the owner of this server?': ['eric'],
                 'Who is the editor of this server?': ['mr.smoothy', 'nk2'], 
...
tic = 'tickets:' #ba.charstr(ba.SpecialChar.TICKET) #not working in 1.5
uni = {'\\c': '\\ue043','\\sh': '\\ue049','\\v':'\\ue04c', '\\h': '\\ue047', '\\z':'\\ue044', '\\n': '\\ue04b', '\\b': '\\ue04b', '\\d': '\\ue048', '\\s': '\\ue046', '\\f': '\\ue04f', '\\l': '\\ue00c', '\\g': '\\ue02c', '\\t': '\\ue02f', '\\i': '\\ue03a'}
