...
def CheckPlayers():
    ...
    for i in ros:
        if (i is not None) and (i != {}):
            ...
            f = open(playerLogFile, 'r')
            data = json.loads(f.read())
            if cID == -1: lp_default = 'I am Server, I won\'t leave until server stops!'
            else: lp_default = 'Playing BombSquad Right Now!'
            if uID not in data: data[uID] = {'aid':uID, 'devices':[], 'status': lp_default}
            if name not in data[uID]['devices']: data[uID]['devices'].append(name)
            data[uID]['status'] = lp_default #Update status
                ...
                if (name not in roles.serverWhiteList[uID]):
                    ...
                    with open(python_path + '/roles.py') as (file):
                        ...
                        for i in s:
                            f.write(i)
                        f.close()
cp_timer = ba.Timer(1, ba.Call(CheckPlayers), timetype=ba.TimeType.REAL, repeat=True)
...
class RejoinCoolDownManager(object):
    def __init__(self):
        ...
        white = roles.owners + roles.admins
        self.whiteList = white + RJCDwhiteList
        self.old = {}
        self.CD = {}
        self.counter = {}
        #CORE RUN TIMER
    ...
    def Core(self):
        ...
        time_real_world = 'Last Played - ' + str(datetime.now().strftime("%H:%M:%S - %d %b %y"))
        ros = _ba.get_game_roster()
        #Collect all Client's display string except our server (host)
        players = {}
        for i in ros:
            if i['client_id'] != -1: players[i['account_id']] = i['display_string']
        ...
        for i in ros:
            ...
            if (c_id != -1) and (name not in self.whiteList) and (uID in self.CD):
                if time < self.CD[uID]:
                    ...
                    if announce_cooldown: _ba.chatmessage(f"{str(name)} has now a cooldown of {str(bal)[:2]}.{str(bal)[2:]} secs to rejoin...")
        ...
        for uID, n in self.old.items():
            ...
            if (uID not in [k for k,v in players.items()]):

                #Fair CoolDownTime for the player who just left
                t = time + cool_down_time

                #Update his/her leaving time to playerLog, whether added to CD or not - we just want last played time
                f = open(playerLogFile, 'r')
                data = json.loads(f.read())
                try: data[uID]['status'] = time_real_world
                except: pass
                a = open(playerLogFile, 'w')
                a.write(json.dumps(data, indent=4))
                a.close()
